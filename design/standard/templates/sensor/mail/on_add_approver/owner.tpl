            ...
            <td align='left' style='background: #ffffff; border: 1px solid #dce1e5;' valign='top' width=''>
                <table border='0' cellpadding='0' cellspacing='0' width='100%'>
                    ...
                    {if and( count( $event_details )|gt(0), is_set( $event_details.approvers ) )}
                        <tr>
                            <td align='left' style='border-top: 1px solid #dce1e5;border-bottom: 1px solid #dce1e5;' valign='top'>
                                <p><strong>{"Riferimento per il cittadino"|i18n('sensor/mail/post')}:</strong> {foreach $event_details.approvers as $approver_id}{fetch( content, object, hash( object_id, $approver_id )).name|wash()}{delimiter}, {/delimiter}{/foreach}</p>
                            </td>
                        </tr>
                    {/if}
                    <tr>
