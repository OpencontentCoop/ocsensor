            ...
            <div class="panel-body">
                ...
                <div class="tab-content">
                    ...
                    {foreach $content_attributes_grouped_data_map as $attribute_group => $content_attributes_grouped}
                        ...
                        <div class="clearfix attribute-edit tab-pane{if $count|eq(0)} active{/if}"
                             id="attribute-group-{$attribute_group}">
                            {set $count = $count|inc()}
                                                ...
                                                <div class="col-md-3">
                                                    {if and( $contentclass_attribute.data_type_string|ne('ezboolean'), $contentclass_attribute.identifier|ne('privacy'), $contentclass_attribute.identifier|ne('on_behalf_of_detail') )}
                                                        <p{if $attribute.has_validation_error} class="message-error"{/if}>
                                                            <span style="white-space: nowrap">{first_set( $contentclass_attribute.nameList[$content_language], $contentclass_attribute.name )|wash}{if $attribute.is_required}
                                                                <span class="required"
                                                                      title="{'required'|i18n( 'design/admin/content/edit_attribute' )}">
                                                                        *</span>{/if}</span>
                                                            {if $attribute.is_information_collector} <span
                ...
                {section show=or( $validation.attributes, $validation.placement, $validation.custom_rules )}
                    <div class="alert alert-warning alert-dismissible" role="alert">
                        ...
                        {section show=$validation.attributes}
                            <p>{'Required data is either missing or is invalid'|i18n( 'design/admin/content/edit' )}
                                :</p>
                            <ul class="list-unstyled">
                                {section var=UnvalidatedAttributes loop=$validation.attributes}
                                    <li><strong>{$UnvalidatedAttributes.item.name|wash}
                                            :</strong> {$UnvalidatedAttributes.item.description}</li>
                                {/section}
                            </ul>
                        {/section}
                        ...
                        {section show=$validation.custom_rules}
                            <p>{'The following data is invalid according to the custom validation rules'|i18n( 'design/admin/content/edit' )}
                                :</p>
                            <ul class="list-unstyled">
                ...
                <div class="buttonblock">
                    <input class="btn btn-lg btn-success pull-right" type="submit" name="PublishButton"
                           value="{'Salva'|i18n('sensor/add')}"/>
                    <input class="btn btn-lg btn-danger" type="submit" name="DiscardButton"
                           value="{'Annulla'|i18n('sensor/add')}"/>
