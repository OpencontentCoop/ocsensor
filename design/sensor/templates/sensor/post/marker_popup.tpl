...
<ul class="list-unstyled">
    ...
    <li><small><i class="fa fa-user"></i> {'In carico a'|i18n('sensor/post')} {$sensor_post.current_owner|wash()}</small></li>
    {/if}
    <li><small><i class="fa fa-comment"></i> {$sensor_post.comment_count} {'commenti'|i18n('sensor/post')}</small></li>
	<li><small><i class="fa fa-comment"></i> {$sensor_post.response_count} {'risposte ufficiali'|i18n('sensor/post')}</small></li>
</ul>
<p><a href={concat('sensor/posts/',$sensor_post.id)|ezurl()} class="pull-right btn btn-info btn-sm" style="color:#fff">{"Dettagli"|i18n('sensor/dashboard')}</a></p>
</div>