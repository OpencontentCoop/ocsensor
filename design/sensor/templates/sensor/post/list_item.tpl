  ...
  <div class="col-md-12">
	<section class="hgroup">
	  ...
	  <li>
		...
		<span class="label label-{$sensor_post.current_object_state.css_class}">{$sensor_post.current_object_state.name|wash()}</span>
		{if $sensor_post.current_privacy_state.identifier|eq('private')}
		  <span class="label label-{$sensor_post.current_privacy_state.css_class}">{$sensor_post.current_privacy_state.name|wash()}</span>
		{/if}
		{if $sensor_post.current_moderation_state.identifier|eq('waiting')}
		  <span class="label label-{$sensor_post.current_moderation_state.css_class}">{$sensor_post.current_moderation_state.name|wash()}</span>
		{/if}
		</li>
	  </ul>
	</section>
...
<div class="row service_teaser" style="margin-bottom: 10px;">
	...
	<div class="service_details {if $sensor_post.object|has_attribute('image')}col-sm-8 col-md-8{else}col-sm-12 col-md-12{/if}">
	  ...
	  <ul class="list-inline">
		  ...
		  {if $sensor_post.comment_count|gt(0)}<li><small><i class="fa fa-comments"></i> {$sensor_post.comment_count} {'commenti'|i18n('sensor/post')}</small></li>{/if}
		  {if $sensor_post.response_count|gt(0)}<li><small><i class="fa fa-comment"></i> {$sensor_post.response_count} {'risposte ufficiali'|i18n('sensor/post')}</small></li>{/if}
